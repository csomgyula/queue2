 ...
 */
public class Sequence2G {
    public static class Result{
        public int value;
        public State actualState, previousState;
        public boolean won, otherWasActive;

        public Result(int value, State previousState, State actualState, boolean won,
                      boolean otherWasActive) {
            this.value = value;
            this.previousState = previousState;
            this.actualState = actualState;
            this.otherWasActive = otherWasActive;
            this.won = won;
        }
    }
    ...
    public Result next() {
        ...
        if (setSucceeded){
            Result result =
                new Result(nextValue, actualState, newState, true, otherIsActive);
        ...
        // 5.2 accept its offer otherwise
            ...
            newState = state.get();
            int itsValue = newState.getValue();
            Integer itsOffer = newState.getOffer();

            int myValue = itsOffer != null ? itsOffer : itsValue + 1;

            deactivateMe(myId, newState);

            // FIXME:
            // if myValue was itsValue + 1, try to incerement state.value
            // nullify offer if any
            Result result = new Result(myValue, actualState, newState, false, otherIsActive);
    ...
    protected void deactivateMe(int myId, State state){
        switch (myId) {
            case 0: state.getActive0().setValue(false); break;
            case 1: state.getActive1().setValue(false); break;
            default:
                throw new IllegalArgumentException("Illegal thread index: " + myId);
        }
    }
    ...
    protected boolean isOtherActive(int myId, State state) {
        return isActive((myId + 1) % 2, state);
    }

    protected boolean isActive(int id, State state) {
        switch (id) {
            case 0:
            ...
            default:
                throw new IllegalArgumentException("Illegal thread index: " + id);
        }
    }
